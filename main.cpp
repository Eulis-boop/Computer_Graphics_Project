...
float rotation_angle = 0.0f, radius = 5.0f;
...
void handleKeypress() {
	...
	if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) rotation_angle += 0.2f / 100; //Adjust speed of rotation
	if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) rotation_angle -= 0.2f / 100; //Decrease the angle
...
void display(void)
{
	...
	//Update the camera position around the y-axis
	cam_pos_x = radius * cos(rotation_angle);
	cam_pos_z = radius * sin(rotation_angle);
